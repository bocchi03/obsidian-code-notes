...
![image-20250522194219481](../image/image-20250522194219481.png)
...
![image-20250522194326627](../image/image-20250522194326627.png)
...
![image-20250522194408883](../image/image-20250522194408883.png)
...
- **操作**:
  ...
  ![image-20250522194526424](../image/image-20250522194526424.png)
...
- **实现**: 通常是一个小的组合逻辑电路（可以用真值表描述）。例如：
  ...
  ![image-20250522194748240](../image/image-20250522194748240.png)

### 4.4.2 主控制单元的设计 (Design of the main control unit)
- **目的**: 根据当前指令的操作码（opcode）生成所有必需的控制信号，以协调数据通路中各个组件的操作。

![image-20250522194831956](../image/image-20250522194831956.png)
...
![image-20250522195009861](../image/image-20250522195009861.png)
...
- **资源利用率低**:
  ...
  ![image-20250522195314287](../image/image-20250522195314287.png)
...
- **流水线**: 当第一批衣服进入烘干（D）阶段时，第二批衣服可以开始洗衣（W）。当第一批进入折叠（F），第二批进入烘干（D），第三批可以开始洗衣（W）。
...
![image-20250522195807854](../image/image-20250522195807854.png)
...
- **解决方法**:
  ...
  ![image-20250522200701031](../image/image-20250522200701031.png)
  
  - **数据前推/旁路 (Forwarding/Bypassing)**: 将计算结果从产生它的流水线阶段（如EX或MEM的输出）直接传递给需要它的后续指令的输入（如EX的输入），而无需等待结果写回寄存器堆再读出。
  
  ![image-20250522200504680](../image/image-20250522200504680.png)
...
![image-20250522200932523](../image/image-20250522200932523.png)

### 流水线寄存器 (Pipeline Registers)
- **作用**: 这是流水线设计的核心。在流水线的每个主要阶段之间插入寄存器，用于存储从前一阶段传递给后一阶段的所有数据和控制信号。它们在每个时钟周期结束时锁存新的值。

![image-20250522201038999](../image/image-20250522201038999.png)
...
- **内容**:
  ...
  ![image-20250526195231270](../image/image-20250526195231270.png)
...
![image-20250526195247758](../image/image-20250526195247758.png)

- **单周期流水线数据通路图 (Single-cycle diagram of pipelined datapath)**: 显示了完整的五级流水线数据通路，包括所有功能单元和连接它们的流水线寄存器。这种图有助于理解数据和控制信号如何在各级之间流动。

![image-20250526195303556](../image/image-20250526195303556.png)

### 4.7.2 流水线控制 (Pipelined control)

![image-20250526195348056](../image/image-20250526195348056.png)
...
![image-20250526195615783](../image/image-20250526195615783.png)
...
- **原因**: 当旁路无法完全解决数据冒险时，流水线必须暂停。最典型的例子是 Load-Use Hazard:
  ...
  ![image-20250528194536748](../image/image-20250528194536748.png)
...
![image-20250528194703157](../image/image-20250528194703157.png)
...
- **分支历史表 (Branch History Table - BHT / Branch Prediction Buffer - BPB)**:
  ...
  ![image-20250528194832268](../image/image-20250528194832268.png)
...
![image-20250528195005880](../image/image-20250528195005880.png)
...
![image-20250528195149618](../image/image-20250528195149618.png)
...
![image-20250528195248299](../image/image-20250528195248299.png)
